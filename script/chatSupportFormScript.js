window.onload = function () {
	...
	this.console.log("the project id is: "+projectId);
	if (projectId && projectId != null && projectId != "" && widgetName ) {
		configTheLoader(true);
		addSingleWidgetConfiguration(projectId,widgetName,domainName,imageLogo,widgetThemeColor);
	...
	document.querySelector("#cwa-login").onclick = function () {
		...
		var popupWindow = window.open('http://access.answerforce.com/?siteUrl=' + siteUrl, "_blank", 'scrollbars=yes,resizable=0,width='+windowWidth+', height='+windowHeight+', top='+posTop+', left='+posLeft+'');
		popupWindow.focus();
	...
	function addSingleWidgetConfiguration(projectId,widgetName,domainName,imageLogo,widgetThemeColor){
		...
		singleWidget.insertAdjacentHTML('beforeend', '<div style="cursor:default" id='+projectId+'><figure style="background-color:#'+widgetThemeColor+'"><cite style="border-color:#'+widgetThemeColor+'"></cite>'+
	...
	function saveWidgetConfiguration(projectId,widgetName,domainName,imageLogo,widgetThemeColor){
	...
	function addMultipleWidgetConfiguration(widgetData){
		...
		var projectId = widgetData.id;
		var widgetThemeColor = widgetData.widgetThemeColor;
		var isImageAvaiable = (imageLogo && imageLogo != "https://assets.chatsupport.co/chat/images/icon-chat.svg") ? "block" :"none";
		var isDomainAvailable = (domainName) ? "block" : "none";
		var d1 = document.querySelector('#multiple-widget-setup section');
		d1.insertAdjacentHTML('beforeend', '<aside><div id='+projectId+'><figure style="background-color:#'+widgetThemeColor+'"><cite style="border-color:#'+widgetThemeColor+'"></cite>'+
		'<img style="display:'+isImageAvaiable+'" src="'+imageLogo+'"/></figure>'+
		'<label>'+widgetName+'</label>'+
		'<span style="display:'+isDomainAvailable+'">'+domainName+'</span></div></aside>');
			if(d1.querySelector("aside")){
			d1.querySelector('aside #'+projectId).onclick = function(){
				configTheLoader(true);
